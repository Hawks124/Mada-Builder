  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected
          ? selectedClass
          : cn("border-border/40 bg-muted/20 text-foreground", hoverClass),
      )}
    >
      {IconComp && (
        <IconComp
...
export function FilterSheet() {
  ...
  const displayedCats = showAllCats
    ? PRODUCT_CATEGORIES
    : PRODUCT_CATEGORIES.slice(0, 9);
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-7">
              ...
              <FilterGroup title="Catégories">
                ...
                >
                  {showAllCats
                    ? "Voir moins"
                    : `+ ${PRODUCT_CATEGORIES.length - 9} de plus`}
                  <CaretDownIcon
                    weight="bold"
                    className={cn(
                      "w-3 h-3 transition-transform",
                      showAllCats && "rotate-180",
                    )}
                  />
                </button>
              </FilterGroup>
              ...
              <FilterGroup title="Plateformes">
                {PLATFORMS.map((p: Platform) => (
                  <FilterChip
                    ...
                    onClick={() =>
                      toggleMulti(selectedPlatforms, setSelectedPlatforms, p.id)
                    }
                  />
                ))}
              </FilterGroup>
              ...
              <FilterGroup title="Modèle économique">
                {PRICING_MODELS.map((p: PricingModel) => (
                  <FilterChip
                    ...
                    onClick={() =>
                      toggleMulti(selectedPricing, setSelectedPricing, p.id)
                    }
                  />
                ))}
              </FilterGroup>
              ...
              <FilterGroup title="Type de produit">
                {PRODUCT_TYPES.map((t: ProductType) => (
                  <FilterChip
                    ...
                    onClick={() =>
                      toggleMulti(selectedTypes, setSelectedTypes, t.id)
                    }
                  />
                ))}
              </FilterGroup>
              ...
              <FilterGroup title="Classification d'âge">
                {AGE_RATINGS.map((r: AgeRating) => (
                  <FilterChip
                    ...
                    onClick={() =>
                      toggleMulti(selectedAges, setSelectedAges, r.id)
                    }
                  />
                ))}
              </FilterGroup>
              ...
              <FilterGroup title="Avancement">
                {LIFECYCLE_STATUS.map((l: LifecycleStatus) => (
                  <FilterChip
                    ...
                    onClick={() =>
                      toggleMulti(selectedLifecycle, setSelectedLifecycle, l.id)
                    }
