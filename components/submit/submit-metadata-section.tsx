...
export function SubmitMetadataSection() {
  ...
  const isMonetized = ["freemium", "paid", "subscription"].includes(
    selectedPricing,
  );
    ...
    <div className="flex flex-col gap-10">
      <div className="flex flex-col gap-1">
        <h2 className="text-2xl font-black tracking-tight">
          Détails techniques & modèle
        </h2>
        <p className="text-[14px] font-medium text-muted-foreground">
          Définissez sur quelles plateformes votre produit fonctionne et comment
          il est monétisé.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div className="md:col-span-2 pt-4 pb-4 flex flex-col gap-6 relative">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[14px] font-medium text-muted-foreground max-w-xl">
                Connectez une clé API en <strong>lecture seule</strong> pour
                prouver publiquement vos revenus et gagner en crédibilité.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3 mt-1">
                {[
                  { icon: <ShieldCheck weight="fill" className="w-4 h-4 text-emerald-600" />, label: "Chiffrement AES-256" },
                  { icon: <Key weight="bold" className="w-4 h-4 text-emerald-600" />, label: "Lecture seule uniquement" },
                  { icon: <ShieldCheck weight="duotone" className="w-4 h-4 text-emerald-600" />, label: "Aucune donnée client" },
                  { icon: <Key weight="duotone" className="w-4 h-4 text-emerald-600" />, label: "Révocable à tout moment" },
                ].map((item, i) => (
              ...
              <p className="text-[11px] font-medium text-muted-foreground/60 leading-relaxed mt-1">
                Sur Stripe : créez une clé restreinte avec la permission <span className="font-mono bg-muted/50 px-1 rounded">Lire les charges &amp; abonnements</span> uniquement. Votre clé est chiffrée dès réception et jamais partagée.
              </p>
            </div>
            )}
          </div>
        ...
        <InputField 
          ...
          subtitle={isDevFacing ? "Requise pour ce type de produit : comment l'installer ?" : "Pour les packages ou outils CLI."}
      ...
      <div className="flex flex-col gap-1 mb-2">
        <h2 className="text-2xl font-black tracking-tight">
          Transparence & Privacy
        </h2>
        <p className="text-[14px] font-medium text-muted-foreground">
          Informez vos utilisateurs des éléments tiers inclus dans votre
          produit.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[12px] font-medium text-muted-foreground group-hover:text-foreground/70 transition-colors">
              Ce produit partage des données via des partenaires tiers (Ads,
              Analytics).
            </span>
          </div>
          <button
