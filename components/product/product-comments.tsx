  ...
  return (
    <div className="flex flex-col gap-8 pt-10 border-t border-border/40" id="comments">
      ...
      <div className="flex gap-3 items-start">
        <AvatarImage src="https://i.pravatar.cc/150?u=me" name="Moi" size={36} className="mt-0.5" />
      ...
      <div className="flex flex-col gap-6">
        {MOCK_COMMENTS.map((comment) => (
          <div key={comment.id} className="flex flex-col gap-3">
            ...
            <div className="flex gap-3 items-start">
              <Link
                href={`/makers/${comment.id}`}
                className="shrink-0 hover:opacity-75 transition-opacity"
              >
                <AvatarImage src={comment.avatar} name={comment.author} size={36} />
              </Link>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <div className="flex items-center bg-muted/40 rounded-full p-0.5">
                    <button className="p-1.5 hover:bg-background rounded-full transition-colors group cursor-pointer">
                      <CaretUpIcon
                        weight="bold"
                        className="w-3 h-3 text-muted-foreground group-hover:text-emerald-500 transition-colors"
                      />
                    </button>
                    <span className="text-[10px] font-black text-foreground px-1">
                      {comment.upvotes}
                    </span>
                    <button className="p-1.5 hover:bg-background rounded-full transition-colors group cursor-pointer">
                      <CaretDownIcon
              ...
              <div className="ml-12 flex flex-col gap-3 border-l-2 border-border/20 pl-4">
                {comment.replies.map((reply) => (
                  <div key={reply.id} className="flex gap-3 items-start">
                    <Link
                      href={`/makers/${reply.id}`}
                      className="shrink-0 hover:opacity-75 transition-opacity"
                    >
                      <AvatarImage src={reply.avatar} name={reply.author} size={32} />
                    </Link>
